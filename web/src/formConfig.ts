...
export const FORM_CONFIG: Record<string, FieldDef[]> = {
  ...
  'stock-control-parameters': [
    ...
    { name: 'customerPriority', label: 'Müşteri Önceliği (küçük = önce çalışılır)', type: 'number' },
    { name: 'shipmentPriority', label: 'Sevkiyat Önceliği (eşitlikte kullanılır)', type: 'number' },
    { name: 'isActive', label: 'Aktif', type: 'bool' },
  ],
  'document-planning-parameters': [
    { name: 'businessPartnerId', label: 'Cari (boş = tüm cariler)', type: 'ref', refResource: 'partners' },
    { name: 'partCount', label: 'Parça Sayısı (en az 2)', type: 'number' },
    ...
    { name: 'planningOperationTypeId', label: 'Planlama Operasyonu (parçaların operasyonu)', type: 'ref', refResource: 'operation-types' },
    { name: 'locationAssign', label: 'Lokasyon Atama (parçalara kaynak/hedef doldur)', type: 'bool' },
    { name: 'isActive', label: 'Aktif', type: 'bool' },
  ],
  ...
  'work-order-general-parameters': [
    { name: 'businessPartnerId', label: 'Cari (boş = tüm cariler)', type: 'ref', refResource: 'partners' },
    { name: 'alarmDuration', label: 'Alarm Süresi (bu süredir açık iş emri uyarı üretir)', type: 'number' },
    {
      name: 'alarmUnit', label: 'Alarm Süre Birimi', type: 'select',
      options: [{ value: 1, label: 'Dakika' }, { value: 2, label: 'Saat' }, { value: 3, label: 'Gün' }],
    },
  ...
  'work-order-reference-operations': [
    {
      name: 'category', label: 'İş Emri Türü', type: 'select', required: true,
      options: [{ value: 1, label: 'Toplama' }, { value: 2, label: 'Yerleştirme' }, { value: 3, label: 'Sayım' }, { value: 4, label: 'Transfer' }, { value: 5, label: 'Raf Besleme' }],
    },
    { name: 'operationTypeId', label: 'Operasyon Tipi (hareketi bu operasyon üretir)', type: 'ref', required: true, refResource: 'operation-types' },
    { name: 'businessPartnerId', label: 'Cari (boş = tüm cariler)', type: 'ref', refResource: 'partners' },
    { name: 'isActive', label: 'Aktif', type: 'bool' },
  ],
