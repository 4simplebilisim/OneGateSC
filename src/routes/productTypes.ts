...
export async function productTypeRoutes(app: FastifyInstance) {
  ...
  app.get('/:id', async (request, reply) => {
    ...
    const row = await prisma.tBLPRODUCTTYPE.findFirst({ where: { id, companyId: getCompanyId(request) } })
  ...
  app.patch('/:id', { preHandler: [app.authenticate, app.requireWrite] }, async (request, reply) => {
    ...
    const existing = await prisma.tBLPRODUCTTYPE.findFirst({ where: { id, companyId: getCompanyId(request) } })
