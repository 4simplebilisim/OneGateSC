...
export async function locationCapacityRoutes(app: FastifyInstance) {
  app.get('/', async (request) =>
    prisma.tBLLOCATIONCAPACITY.findMany({ where: { companyId: getCompanyId(request) }, orderBy: { id: 'desc' } }),
  )
  ...
  app.patch('/:id', { preHandler: [app.authenticate, app.requireWrite] }, async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'Invalid body', details: parsed.error.flatten() })
    const existing = await prisma.tBLLOCATIONCAPACITY.findFirst({ where: { id, companyId: getCompanyId(request) } })
  ...
  app.delete('/:id', { preHandler: [app.authenticate, app.requireWrite] }, async (request, reply) => {
    const id = Number((request.params as { id: string }).id)
    if (!Number.isInteger(id)) return reply.code(400).send({ error: 'Invalid id' })
    const res = await prisma.tBLLOCATIONCAPACITY.deleteMany({ where: { id, companyId: getCompanyId(request) } })
