...
export async function productGroupRoutes(app: FastifyInstance) {
  app.get('/', async (request) => {
    return prisma.tBLPRODUCTGROUP.findMany({ where: { ...companyListFilter(request) }, orderBy: { code: 'asc' } })
  })
  ...
  app.patch('/:id', { preHandler: [app.authenticate, app.requireWrite] }, async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'Invalid body', details: parsed.error.flatten() })
    const existing = await prisma.tBLPRODUCTGROUP.findFirst({ where: { id, ...companyListFilter(request) } })
