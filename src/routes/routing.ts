...
export async function routingRuleRoutes(app: FastifyInstance) {
  ...
  app.get('/:id', async (request, reply) => {
    ...
    const row = await prisma.tBLROUTINGRULE.findFirst({ where: { id, companyId: getCompanyId(request) } })
  ...
  app.patch('/:id', { preHandler: [app.authenticate, app.requireWrite] }, async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'Invalid body', details: parsed.error.flatten() })
    const existing = await prisma.tBLROUTINGRULE.findFirst({ where: { id, companyId: getCompanyId(request) } })
  ...
  app.delete('/:id', { preHandler: [app.authenticate, app.requireWrite] }, async (request, reply) => {
    const id = Number((request.params as { id: string }).id)
    if (!Number.isInteger(id)) return reply.code(400).send({ error: 'Invalid id' })
    const res = await prisma.tBLROUTINGRULE.deleteMany({ where: { id, companyId: getCompanyId(request) } })
