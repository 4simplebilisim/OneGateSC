...
export async function sequenceRoutes(app: FastifyInstance) {
  app.get('/', async (request) => {
    return prisma.tBLSEQUENCE.findMany({ where: { companyId: getCompanyId(request) }, orderBy: { code: 'asc' } })
  })
  ...
  app.patch('/:id', { preHandler: [app.authenticate, app.requireWrite] }, async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'Invalid body', details: parsed.error.flatten() })
    const existing = await prisma.tBLSEQUENCE.findFirst({ where: { id, companyId: getCompanyId(request) } })
