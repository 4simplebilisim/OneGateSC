...
export async function vehicleRoutes(app: FastifyInstance) {
  app.get('/', async (request) => {
    return prisma.tBLVEHICLE.findMany({ where: { ...companyListFilter(request) }, orderBy: { plateNo: 'asc' } })
  })

  app.get('/:id', async (request, reply) => {
    const id = Number((request.params as { id: string }).id)
    if (!Number.isInteger(id)) return reply.code(400).send({ error: 'Invalid id' })
    const vehicle = await prisma.tBLVEHICLE.findFirst({ where: { id, ...companyListFilter(request) } })
  ...
  app.patch('/:id', { preHandler: [app.authenticate, app.requireWrite] }, async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'Invalid body', details: parsed.error.flatten() })
    const existing = await prisma.tBLVEHICLE.findFirst({ where: { id, ...companyListFilter(request) } })
